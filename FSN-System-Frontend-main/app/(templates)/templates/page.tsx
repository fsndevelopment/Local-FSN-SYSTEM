...
export default function TemplatesPage() {
  ...
  const handleTemplateExecution = async (request: any) => {
    try {
      ...
      setSuccessMessage(`Failed to start ${selectedTemplate?.name}: ${(error as Error).message}`)
      setShowSuccess(true)
    }
  }
  ...
  const handleStopTemplate = async (template: Template) => {
    try {
      ...
      setSuccessMessage(`Failed to stop ${template.name}: ${(error as Error).message}`)
      setShowSuccess(true)
    }
  }
  ...
  return (
    <LicenseBlocker action="access templates">
              ...
              <div className="mt-8">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex-1 max-w-md">
                    <GlobalSearchBar placeholder="Search templates..." />
                  </div>
      ...
      <TemplateExecutionDialog
        ...
        accounts={(accounts?.data?.items || []) as any}
