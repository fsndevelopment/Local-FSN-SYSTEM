...
export function Sidebar() {
  ...
  return (
    <div 
      className={cn(
        "fixed left-4 top-4 bottom-4 bg-black rounded-2xl flex flex-col py-6 shadow-lg z-50 transition-all duration-300",
          ...
          return (
            <div key={item.name} className="relative flex items-center">
              <Link
                href={item.href}
                className={cn(
                  "relative flex items-center w-full group transition-all duration-200 rounded-full",
                  !isActive && "hover:bg-neutral-800 hover:scale-105 transform",
                  isActive && isExpanded ? "bg-white" : ""
                )}
              >
                <div className={cn(
                  "w-10 h-10 rounded-full flex items-center justify-center transition-all duration-200 group relative flex-shrink-0 z-10",
