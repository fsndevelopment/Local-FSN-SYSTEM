...
class LicenseAwareStorageService {
  ...
  async getTemplates(): Promise<LocalTemplate[]> {
    ...
    const storedTemplates = this.getItem<LocalTemplate[]>(storageKey) || []
  ...
  async updateTemplate(templateId: string, updates: Partial<LocalTemplate>): Promise<void> {
    try {
      ...
      // Update localStorage
      const templates = this.getItem<LocalTemplate[]>(STORAGE_KEYS.TEMPLATES) || []
  ...
  async deleteTemplate(templateId: string): Promise<void> {
    try {
      ...
      this.setItem(STORAGE_KEYS.TEMPLATES, filtered)
  ...
  async getWarmupTemplates(): Promise<LocalWarmupTemplate[]> {
    try {
      // For now, use localStorage directly since warmup templates have different structure
      return this.getItem<LocalWarmupTemplate[]>('warmupTemplates') || []
    } catch (error) {
