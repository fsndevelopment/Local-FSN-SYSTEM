...
from services.crane_account_switching import crane_instagram_account_switching
...
class CraneAccountSwitchingTest:
    ...
    async def test_individual_crane_steps(self, container_id: str):
        ...
        # Get driver first
        driver = await crane_instagram_account_switching._get_driver(self.device_udid, self.platform)
        if not driver:
        ...
        for step_method, step_name in steps:
            ...
            try:
                if step_method == "navigate_to_home_screen":
                    result = await crane_instagram_account_switching._navigate_to_home_screen(self.device_udid, driver, wait)
                elif step_method == "long_press_app_icon":
                    result = await crane_instagram_account_switching._long_press_app_icon(self.device_udid, self.platform, driver, wait)
                elif step_method == "click_container_default":
                    result = await crane_instagram_account_switching._click_container_default_button(self.device_udid, driver, wait)
                elif step_method == "select_target_container":
                    result = await crane_instagram_account_switching._select_target_container(self.device_udid, container_id, driver, wait)
                elif step_method == "launch_app_with_container":
                    result = await crane_instagram_account_switching._launch_app_with_container(self.device_udid, self.platform, driver, wait)
                else:
    ...
    async def run_comprehensive_test(self):
        ...
        print("🚀 Starting Instagram Crane Account Switching Test")
        print("=" * 60)
...
async def main():
    ...
    print("🔧 Crane Account Switching Test (Instagram)")
